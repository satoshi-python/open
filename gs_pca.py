            ...
            for j2 in range(len(x1)):
                if int(kizami -(int((y1[j2] - y_min)/y_haba) + 1)) == int(x2[j][j1]):
                    if int((int((x1[j2] - x_min)/x_haba) - 1)) == int(y2[j][j1]):
                        if j2 not in num:
                            num.append(j2)
                        num2 = int(kizami -(int((y1[j2] - y_min)/y_haba) + 1))
                        num3 = int((int((x1[j2] - x_min)/x_haba) - 1))
            kaki_x(savepath,num,num1,num2,num3)
        print("end_YY")
def kaki_x(savepath,num,num1,num2,num3):
    f = open("{0}/{1}({2}_{3}).txt".format(savepath,str(num1),str(num2),str(num3)),"w")
...
def free_energy(path,kizami,savepath,probpath,FONT=int(14),ligand="satoshi"):
    ...
    plt.xticks([0,kizami/4,kizami/2,kizami*3/4,kizami],[round(x_min,2),round(x_min+(x_haba * kizami/4),2),0.00,round(x_max-(x_haba * kizami/4),2),round(x_max,2)])
    plt.yticks([0,kizami/4,kizami/2,kizami*3/4,kizami],[round(y_max,2),round(y_max-(y_haba * kizami/4),2),0.00,round(y_min+(y_haba * kizami/4),2),round(y_min,2)])
